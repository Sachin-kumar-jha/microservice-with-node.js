  ...
  await consumer.run({
    eachMessage: async ({ message }) => {
      ...
      try {
        ...
        console.error("Invalid Kafka message:", message.value.toString(), err);
        return;
      }

      // Instead of sending email, just log the message
      console.log("✅ Dummy notification received:", evt);
    }
  });
}
...
async function startService() {
  await startConsumer();
  app.listen(port, () => console.log(`✅ Dummy notification service running on port ${port}`));
}
